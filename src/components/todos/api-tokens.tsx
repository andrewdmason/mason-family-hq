...
import { createApiToken, revokeApiToken } from "@/app/(todos)/todos/actions";
...
/**
 * Personal API tokens for /api/todo/ingest (the iOS Reminders Shortcut and
 * the Raycast extension). The raw token is displayed exactly once, right
 * after minting — copy it then; only its hash is stored.
 */
        ...
        {tokens.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              No tokens yet. Create one for the iOS Shortcut or Raycast.
            </p>
          </div>
        ) : (
                ...
                <tr key={token.id} className="border-t border-border/60">
                  ...
                  <td className="px-3 py-2.5 text-right">
                    <button
                      type="button"
                      onClick={() =>
                        revokeApiToken(token.id).finally(() => router.refresh())
                      }
