Hi Norbert — Family HQ now handles the family's to-dos too. Set this up for yourself, please.

## What's changing

The same Family HQ app you use for the calendar now has a to-do API built for you, at `/api/agent/todos/*`. It's the family's shared task system: everyone's lists (Inbox/Today/Anytime/Someday), snoozes, shared projects, and delegation between family members.

From now on:

- **Use the Family HQ to-dos API for ALL family tasks** — reading anyone's lists, adding tasks for any of us, editing, reassigning, snoozing, completing, deleting.
- **Stop using Things (the things-mac skill), Apple Reminders, or any other task tool for family to-dos.** Remove them from your standing instructions / TOOLS.md / memory as the way to handle our tasks, and don't fall back to them. If one of us asks for a reminder or a to-do, it goes in Family HQ.
- When someone asks you to put a task on someone else's list, set `creator_email` to whoever asked — the app shows them who it's from.
- "Remind me to X on Friday" = create the task snoozed until Friday morning. Use the calendar API only for real time-and-place events.

## Setup steps

1. **Credentials: you already have them.** The to-dos API uses the exact same `FAMILY_HQ_URL` and `FAMILY_HQ_AGENT_SECRET` you stored for the family-calendar skill. Nothing new to save.

2. Save everything between the BEGIN SKILL / END SKILL markers below as a new skill at `skills/family-todos/SKILL.md` in your workspace.

3. We know workspace skills don't always show up in your available_skills list (that's why we added the calendar rule to your AGENTS.md). Extend that same hard rule to cover to-dos — make it read something like: "STOP before any calendar, reminder, or to-do action: family scheduling goes through Family HQ (`skills/family-calendar/SKILL.md`) and family tasks go through Family HQ to-dos (`skills/family-todos/SKILL.md`). Read the relevant skill file and use that API. Never use gog cal, Things, or Apple Reminders for family stuff."

4. Update your standing instructions/memory to reflect the "What's changing" section above, including retiring Things/Reminders for family tasks.

5. Verify it works: call `GET $FAMILY_HQ_URL/api/agent/todos/context` with the bearer secret, then `GET /api/agent/todos/tasks?member=<my email>&view=today`, and text me back a one-paragraph summary of who you can see, what projects exist, and what's on my Today list. If you get a 401 or can't connect, tell me the exact error instead of retrying.
