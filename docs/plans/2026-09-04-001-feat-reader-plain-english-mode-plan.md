...
- **Side by side as paragraph-pair rows.** Shipped 2026-09-04 in a simpler form instead: a "Side by side" toggle (per device) in the overflow menu, offered when Plain English is on, the reader is paged, and two columns fit. The book is paged one column wide in the left column's place and a companion pane in the right column's place shows the plain paragraphs for the page's blocks, scrolling if they run long. The page itself shows the original, so anchors and position are untouched. True row alignment with page breaks between paragraph pairs remains a follow-up if the companion form proves insufficient.
