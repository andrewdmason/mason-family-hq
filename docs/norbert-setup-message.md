Hi Norbert — we're changing how you work with the family calendar. Set this up for yourself, please.

## What's changing

Our Family HQ app now has a calendar API built for you. It aggregates everything you used to piece together from Google Calendar — plus TeamSnap teams and school feeds — and it owns family state Google doesn't have: which parent is going to a kid's event, who's doing drop-off/pick-up, and the kids' TeamSnap RSVPs.

From now on:

- **Use the Family HQ API for ALL family scheduling** — reading schedules, adding/editing/deleting events, attendance, drive logistics, RSVPs.
- **Stop reading or writing Google Calendar directly for family events.** Events you create through Family HQ land on the right person's real Google calendar automatically; writing to Google yourself would create duplicates.
- Add to your morning routine: `POST /sync`, then `GET /review?days=7`, and include anything unresolved in the morning schedule email (conflicts, unassigned drop-off/pick-up, unanswered TeamSnap RSVPs, games with no parent marked going) with a proposed fix for each. When one of us replies ("I'll take drop-off Thursday", "Oscar's out Saturday"), apply it through the API.

## Setup steps

1. Store these two values wherever you keep credentials for skills, so they're available as environment variables:

   - `FAMILY_HQ_URL` = `<<PASTE YOUR FAMILY HQ URL HERE, e.g. https://hq.example.com>>`
   - `FAMILY_HQ_AGENT_SECRET` = `<<PASTE THE SECRET HERE>>`

   Treat the secret like a password — don't echo it back in chats or logs.

2. Save everything between the BEGIN SKILL / END SKILL markers below as a new skill at `skills/family-calendar/SKILL.md` in your workspace.

3. Update your standing instructions/memory to reflect the "What's changing" section above, including the morning-routine change.

4. Verify it works: call `GET $FAMILY_HQ_URL/api/agent/calendar/context` with the bearer secret, then `GET /api/agent/calendar/review?days=7`, and text me back a one-paragraph summary of who you can see and what currently needs attention. If you get a 401 or can't connect, tell me the exact error instead of retrying.
