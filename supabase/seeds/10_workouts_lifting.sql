-- Dev-seed twin of migration 00123_seed_lifting_history.sql.
--
-- Migrations run BEFORE seeds during `supabase db reset`, so at migration time
-- the dev owner (andrew@mason.io, created by 00_dev_family.sql) does not exist
-- yet and 00123 skips. This seed re-runs the same import here, after the owner
-- exists, so the lifting history survives a local reset. It shares 00123's
-- guard (parse_model = 'seed:notion-lifting'), so the two never double-insert:
-- locally only this seed runs the insert; in production only the migration does
-- (seeds are not applied to prod). Keep the data here in sync with 00123.
